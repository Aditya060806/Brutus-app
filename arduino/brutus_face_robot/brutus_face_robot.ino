...
#define SERVO_STEP_DEG  3    // degrees per tick (~3° × 125Hz = 375°/s)
...
void runIdleBehaviour() {
  // Cycle expressions every 15 s
  if (millis() - expressionTimer > 15000) {
    int next = expression + 1;
    if (next >= NUM_EXPRESSIONS) next = 0;
    setExpression(next);
    expressionTimer = millis();
  }

  // Random eye saccade every ~2 s
  if (millis() - eyeMoveTimer > 2000) {
    lookAt(random(40, 140), random(50, 130));
    eyeMoveTimer = millis();
  }
