      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            _SectionHeader('Eye Tracking'),
            const SizedBox(height: 12),
            _EyeTrackingCard(state: state),
...
/// Turns the phone's back camera into Brutus's eyes: on-device face detection
/// finds the closest person and the robot's eye servos turn to follow them.
class _EyeTrackingCard extends ConsumerWidget {
  final RobotState state;
  const _EyeTrackingCard({required this.state});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final n = ref.read(robotProvider.notifier);
    final on = state.eyeTrackingOn;
            ...
            children: [
              ...
              const Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Follow with camera',
                        style: TextStyle(
                            fontSize: 15, fontWeight: FontWeight.w600)),
                    SizedBox(height: 2),
                    Text(
                      'Brutus\'s eyes track whoever the back camera sees.',
                      style: TextStyle(
              ...
              Switch(
                value: on,
                onChanged: enabled ? n.setEyeTracking : null,
              ),
            ],
          ),
          if (!enabled)
            const Padding(
              ...
              child: _Hint(text: 'Connect the robot first to use eye tracking.'),
            ),
          if (on) ...[
            const SizedBox(height: 12),
            const _EyeTrackingPreview(),
            ...
            Row(
              children: [
                Container(
                  ...
                  height: 9,
                  decoration: BoxDecoration(
                    color: state.eyeTrackingHasFace
                        ? AppColors.success
                        : AppColors.warning,
                    shape: BoxShape.circle,
                  ),
                ...
                const SizedBox(width: 8),
                Text(
                  state.eyeTrackingHasFace
                      ? 'Tracking a face — eyes following'
                      : 'Looking for a face…',
                  style: const TextStyle(
...
/// Live preview of the eye-tracking camera. Rebuilds when the tracker's state
/// changes (so the [CameraPreview] appears once the controller is ready).
class _EyeTrackingPreview extends StatelessWidget {
  const _EyeTrackingPreview();

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<EyeTrackingState>(
      stream: EyeTrackingService.instance.stateStream,
      builder: (context, _) {
        final controller = EyeTrackingService.instance.controller;
        final running =
            EyeTrackingService.instance.state == EyeTrackingState.running;
        return ClipRRect(
          borderRadius: BorderRadius.circular(12),
          child: (running && controller != null)
              ? AspectRatio(
