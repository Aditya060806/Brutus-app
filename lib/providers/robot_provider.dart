...
import 'package:brutus_app/data/services/eye_tracking_service.dart';
...
export 'package:brutus_app/data/services/eye_tracking_service.dart'
    show EyeTrackingState;
...
class RobotExpression {
  ...
  static const love = 6;
  static const excited = 7;
  static const confused = 8;
  static const scared = 9;
...
class RobotState {
  ...
  final bool eyeTrackingOn;
  final bool eyeTrackingHasFace;
  ...
  const RobotState({
    ...
    this.eyeTrackingOn = false,
    this.eyeTrackingHasFace = false,
    this.lastMessage,
    this.errorMessage,
    this.scanning = false,
  ...
  RobotState copyWith({
    ...
    return RobotState(
      ...
      eyeTrackingOn: eyeTrackingOn ?? this.eyeTrackingOn,
      eyeTrackingHasFace: eyeTrackingHasFace ?? this.eyeTrackingHasFace,
...
class RobotNotifier extends StateNotifier<RobotState> {
  ...
  final EyeTrackingService _eyeTrack = EyeTrackingService.instance;

  // Eye-tracking mapping: how far (in degrees from 90 center) the eyes swing
  // for a target at the edge of the frame.
  static const double _eyeAmpLR = 50; // left/right swing (40..140)
  static const double _eyeAmpUD = 32; // up/down swing (58..122)
  double _smoothLr = 90;
  double _smoothUd = 90;
  DateTime _lastFaceAt = DateTime(2000);
  bool _eyesRecentered = true;
  ...
  StreamSubscription<EyeTarget>? _eyeTargetSub;
  StreamSubscription<EyeTrackingState>? _eyeStateSub;
    ...
    _discoverySub = _bt.discoveryStream.listen((device) {
      ...
      if (idx >= 0) {
        next[idx] = BleDevice(
          ...
          likelyRobot: device.likelyRobot || existing[idx].likelyRobot,
        );
      } else {
        next.add(device);
      }
      next.sort((a, b) {
        // Probable robots (HM-10-style names / FFE0 service) float to the
        // top so the user doesn't hunt through earbuds and watches.
  ...
  // ── Eye tracking: phone back camera → robot eyes ──
  //
  // Runs the phone's back camera through on-device face detection and turns
  // the robot's eyes toward whoever the camera sees. Coexists with auto-drive
  // (which still lip-syncs the mouth + drives expression from Brutus's voice);
  // while tracking is on, the eyes are owned by the tracker.

  Future<void> setEyeTracking(bool on) async {
    if (on) {
      if (!state.isConnected) {
        state = state.copyWith(
          errorMessage: 'Connect the robot first, then start eye tracking.',
        );
        return;
      }
      state = state.copyWith(eyeTrackingOn: true, clearError: true);
      _eyeStateSub ??= _eyeTrack.stateStream.listen(_onEyeState);
      _eyeTargetSub ??= _eyeTrack.targetStream.listen(_onEyeTarget);
      _smoothLr = 90;
      _smoothUd = 90;
      final ok = await _eyeTrack.start();
      if (!ok) {
        state = state.copyWith(
          eyeTrackingOn: false,
          errorMessage: 'Could not start the camera for eye tracking '
              '(check camera permission, and close the chat camera if open).',
        );
      }
    } else {
      state = state.copyWith(eyeTrackingOn: false, eyeTrackingHasFace: false);
      await _eyeTrack.stop();
      if (state.isConnected) {
        if (!state.eyeTrackingOn) _bt.lookAt(lr: 90, ud: 90, force: true); // recenter
      }
    }
  }

  void _onEyeState(EyeTrackingState s) {
    if (s == EyeTrackingState.error && state.eyeTrackingOn) {
      state = state.copyWith(
        eyeTrackingOn: false,
        eyeTrackingHasFace: false,
        errorMessage: 'Eye-tracking camera stopped unexpectedly.',
      );
    }
  }

  void _onEyeTarget(EyeTarget t) {
    if (!state.eyeTrackingOn || !state.isConnected) return;

    if (!t.hasFace) {
      if (state.eyeTrackingHasFace) {
        state = state.copyWith(eyeTrackingHasFace: false);
      }
      // Re-center once after the face has been gone for ~1.2 s.
      if (!_eyesRecentered &&
          DateTime.now().difference(_lastFaceAt) >
              const Duration(milliseconds: 1200)) {
        _eyesRecentered = true;
        _smoothLr = 90;
        _smoothUd = 90;
    ...
    _eyesRecentered = false;
    if (!state.eyeTrackingHasFace) {
      state = state.copyWith(eyeTrackingHasFace: true);
    }

    // Map normalized target → servo angles.
    //   firmware eyeLR: 0=right, 180=left  → target on left (cx→0) looks left
    //   firmware eyeUD: 0=up,    180=down  → target at top (cy→0) looks up
    final targetLr = 90 + (0.5 - t.cx) * 2 * _eyeAmpLR;
    final targetUd = 90 + (t.cy - 0.5) * 2 * _eyeAmpUD;

    // Low-pass smoothing to damp detection jitter.
    _smoothLr += (targetLr - _smoothLr) * 0.4;
    _smoothUd += (targetUd - _smoothUd) * 0.4;

    _bt.lookAt(
      lr: _smoothLr.round().clamp(0, 180),
      ud: _smoothUd.round().clamp(0, 180),
    );
  }
  ...
  void _applyVoiceStatus(VoiceStatus status, {bool force = false}) {
    ...
    switch (status) {
      case VoiceStatus.idle:
        ...
        if (!state.eyeTrackingOn) _bt.lookAt(lr: 90, ud: 90, force: true);
        _bt.closeMouth();
      ...
      case VoiceStatus.thinking:
        _bt.setExpression(RobotExpression.thinking);
        if (!state.eyeTrackingOn) _bt.lookAt(lr: 60, ud: 70, force: true);
        _bt.setLedPattern(RobotLedPattern.pulse);
        state = state.copyWith(
          currentExpression: RobotExpression.thinking,
          ledPattern: RobotLedPattern.pulse,
