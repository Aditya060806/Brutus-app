...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  ///
  /// Routing:
  ///   1. **Primary** — Gemini Live native-audio in Puck/Aoede voice. Returns
  ///      24 kHz mono PCM that we pipe through the existing native AudioTrack
  ///      player, so it sounds *exactly* like Brutus during a live conversation.
  ///   2. **Fallback** — flutter_tts (Android system TTS) if the Gemini call
  ///      fails (no network, quota, empty response). Robotic but always works.
  ///
  ...
  /// unwanted reply. The mic resumes ~1.5 s after the last TTS chunk plays
  /// (enough for the AudioTrack ring buffer to drain).
  Future<void> speakText(String text, {String lang = 'en-US'}) async {
    ...
    // Force a fresh AudioTrack for the TTS turn.
    //
    ...
    if (ok) {
      ...
      // The onIdleStream subscription set up above will handle mic resume
      // once the AudioTrack actually finishes playing.
      return;
    }

    // Gemini failed — cancel the playback-idle waiter (no audio coming
    // through the AudioTrack) and fall back to system TTS, then schedule
    // a fixed resume.
    _speakResumeSub?.cancel();
    _speakResumeSub = null;
    _speakResumeFallback?.cancel();
    ...
    try {
      ...
      _log('Speak-for-me: fell back to system TTS ($lang)');
    } catch (e) {
  ...
  /// Duration to suppress mic audio after the last AI audio chunk.
  /// Must be generous: phone speakers create echo/reverb that persists
  /// 1–2 seconds in a typical room. 2.5s covers worst-case scenarios.
  static const _echoGuardDuration = Duration(milliseconds: 2500);
  ...
  void _listenToGemini() {
    ...
    _geminiSub = _gemini.messages.listen((event) {
      ...
      if (event['type'] == 'disconnected') {
        state = state.copyWith(isConnected: false, status: VoiceStatus.idle);
      }
    });
  }
