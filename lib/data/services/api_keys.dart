...
class ApiKeys {
  ...
  /// Sarvam resolves from secure storage only (no embedded default) — the user
  /// supplies it in Settings → API Keys.
  static Future<String?> sarvam() async {
    try {
      final stored = await SecureStorageService.getSarvamKey();
      if (stored != null && stored.trim().isNotEmpty) return stored.trim();
    } catch (_) {}
    return null;
  }
